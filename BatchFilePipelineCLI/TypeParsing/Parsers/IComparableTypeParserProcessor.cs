...
{
    /// <summary>
    /// Handle the conversions of values to <see cref="IComparable"/>
    /// </summary>
    [Preserve]
    internal sealed class IComparableTypeParserProcessor : ITypeParserProcessor
    {
        /*----------Variables----------*/
        //PRIVATE

        /// <summary>
        /// Cache the type that will be compared against
        /// </summary>
        private readonly Type COMPARABLE_TYPE = typeof(IComparable);
        ...
        public bool CanProcessOutputType(Type targetType) => targetType == COMPARABLE_TYPE;
        ...
        {
            // Do a basic cast operation
            try
            {
                value = (IComparable)inputValue;
                return true;
            }
            ...
            {
                Logger.Exception($"[{nameof(IComparableTypeParserProcessor)}] Failed to cast the input string to type '{targetType}'", ex);
                value = default;
                return false;
            }
        }
