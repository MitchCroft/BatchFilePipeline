...
namespace BatchFilePipelineCLI.Pipeline.Nodes.External.Video.Handbrake
{
    ...
    /// </summary>
    [Node]
    public sealed class TranscodeVideoNode : INode
    {
        /*----------Variables----------*/
        //PRIVATE
        ...
        public async ValueTask<ExecutionResult> ProcessNodeResultAsync(PipelineContext context)
        {
            ...
            if (allowHardwareAcceleration == true)
            {
                ...
                string? hardwareAccelerator = await TryFindSupportedHardwareEncoderAsync(executable, preset, context.CancellationToken);
