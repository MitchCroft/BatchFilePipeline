...
namespace BatchFilePipelineCLI.Pipeline.Nodes.External.Video.FFmpeg
{
    /// <summary>
    /// Handle the extraction of subtitles from a collection of streams
    /// </summary>
    [Node]
    public sealed class ExtractSubtitlesNode : INode
    {
        /*----------Variables----------*/
        //PRIVATE
        ...
        public async ValueTask<ExecutionResult> ProcessNodeResultAsync(PipelineContext context)
        {
            ...
            string storageDir = context.GetInput<string>(_storageProperty);
            string analyseDuration = context.GetInput<string>(_analyseDurationProperty);
            string probeSize = context.GetInput<string>(_probeSizeProperty);
