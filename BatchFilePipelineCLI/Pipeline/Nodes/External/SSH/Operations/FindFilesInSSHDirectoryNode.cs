...
namespace BatchFilePipelineCLI.Pipeline.Nodes.External.SSH.Operations
{
    ...
    /// </summary>
    [Node]
    public sealed class FindFilesInSSHDirectoryNode : INode
    {
        /*----------Variables----------*/
        //PRIVATE
        ...
        public IList<Property> GetInputProperties() => [_clientProperty, _searchDirectoryProperty, _searchPatternProperty, _searchOptionProperty];
        ...
        public async ValueTask<ExecutionResult> ProcessNodeResultAsync(PipelineContext context)
        {
            ...
            string searchDirectory = IOUtility.CleanFilePath(context.GetInput<string>(_searchDirectoryProperty));
            ...
            var fileSearch = searchOption == SearchOption.TopDirectoryOnly ?
                EnumerateRootFilesAsync(client, searchDirectory, context.CancellationToken) :
                EnumerateRecursiveFilesAsync(client, searchDirectory, context.CancellationToken);
