...
namespace BatchFilePipelineCLI.Pipeline.Nodes
{
    ...
    public sealed class NodeLibrary
    {
        ...
        /// </summary>
        private readonly Dictionary<string/*TypeId*/, NodeAttribute> _nodeCharacteristics = new();
        ...
        public bool TryLoadFromTypes(IEnumerable<Type> nodeTypes)
        {
            ...
            foreach (var type in nodeTypes)
            {
                ...
                var characteristic = type.GetCustomAttribute<NodeAttribute>(false);
                if (characteristic is null)
                {
                    Logger.Error($"Unable to process the type '{type}' as a Pipeline Node. Missing the {nameof(NodeAttribute)} attached to describe usage");
                    success = false;
                    continue;
                }
        ...
        public bool TryGetNodeCharacteristics(string typeId, [NotNullWhen(true)] out NodeAttribute? characteristics)
        {
        ...
        public IEnumerable<(Type nodeType, NodeAttribute characteristics)> GetNodeTypes() =>
