...
namespace BatchFilePipelineCLI.Pipeline.Nodes.IO
{
    ...
    /// </summary>
    [Node]
    public sealed class FindFilesInDirectoryNode : INode
    {
        /*----------Variables----------*/
        //PRIVATE
                ...
                {
                    {
                        ...
                        Enumerable.Range(0, filterSegments.Length)
                            .SelectMany(i => Directory.EnumerateFiles(searchDirectory, filterSegments[i], searchOption))
                            .Select(IOUtility.CleanFilePath)
                    }
                }
            ));
        }
