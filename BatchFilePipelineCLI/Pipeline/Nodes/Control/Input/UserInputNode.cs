...
namespace BatchFilePipelineCLI.Pipeline.Nodes.Control.Input
{
    /// <summary>
    /// Allow for the user to enter a string value at runtime for processing
    /// </summary>
    [Node]
    public sealed class UserInputNode : INode
    {
        /*----------Variables----------*/
        //PRIVATE
        ...
        public async ValueTask<ExecutionResult> ProcessNodeResultAsync(PipelineContext context)
        {
            ...
            using var inputCancellationToken = CancellationStack.PushSource(context.CancellationToken);
                                                            ...
                                                            CancellationToken cancellationToken)
        {
            ...
            ConsoleUtility.Write($"{(string.IsNullOrWhiteSpace(defaultValue) ? string.Empty : $"Default Value = \"{defaultValue}\"\n")}{prompt} ");
